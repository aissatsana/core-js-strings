 ...
 */
function removeFirstOccurrences(str1, str2) {
 ...
 */
function removeLastOccurrences(str1, str2) {
...
function isPalindrome(str) {
  ...
  return reverseString(cleanedStr) === cleanedStr;
}
...
function findLongestWord(str) {
  return str.split(' ').reduce((long, curr) => {
 ...
 */
function getStringFromTemplate(str1, str2) {
...
function extractNameFromTemplate(str) {
  return str.replace('Hello, ', '').replace('!', '');
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    const startCode = char >= 'a' ? 97 : 65; // ASCII of 'a' = 97 and 'A' = 65
    const charCode = char.charCodeAt(0);
    return String.fromCharCode(((charCode - startCode + 13) % 26) + startCode);
  });
}
